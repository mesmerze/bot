...
" do
  ...
  scenario "Accessing Sales Dashboard via the nav" do
    visit homepage
    within ".tabs" do
      click_link "Sales Dashboard"
    end

    expect(current_path).to eq(sales_dashboard_page)
  end

  scenario "Viewing Sales Dashboard when all opportunities have been assigned", js: true do
  ...
  scenario "Viewing Sales Dashboard when all opportunities are unassigned", js: true do
  ...
  scenario "Viewing Sales Dashboard when there are no opportunities in the pipeline", js: true do
